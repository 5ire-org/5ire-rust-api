...
impl KeystoreExt for LocalKeystore {
    fn generate<Pair: AppPair>(&self) -> Result<Pair> {
        self.0
            .write()
            .generate_by_type::<Pair::Generic>(Pair::ID)
            .map(Into::into)
    }
    ...
    fn sr25519_generate_new(
        ...
        let pair = match seed {
            Some(seed) => self
                .0
                .write()
                .insert_ephemeral_from_seed_by_type::<sr25519::Pair>(seed, id),
    ...
    fn ed25519_generate_new(
        ...
        let pair = match seed {
            Some(seed) => self
                .0
                .write()
                .insert_ephemeral_from_seed_by_type::<ed25519::Pair>(seed, id),
...
mod tests {
    ...
    impl KeystoreInner {
        ...
        fn generate<Pair: AppPair>(&mut self) -> Result<Pair> {
            self.generate_by_type::<Pair::Generic>(Pair::ID)
                .map(Into::into)
        }
    }
    ...
    fn basic_store() {
        ...
        assert_eq!(
            store.public_keys::<ed25519::AppPublic>().unwrap()[0],
            key.public()
        );
    }
    ...
    fn test_insert_ephemeral_from_seed() {
        ...
        assert!(store
            .key_pair::<ed25519::AppPair>(&pair.public())
            .unwrap()
            .is_none());
    }
